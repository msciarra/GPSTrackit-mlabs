...
def upload_df_to_s3(df_to_upload, file_name, accountId):
    """
    ...
    """
    bucket_name = "proactive-data-analytics"
    df_to_upload.to_csv("/tmp/" + file_name)
    s3_key = f'device_readings_auditing/accountId_auditings/{accountId}/{file_name}'
    s3.upload_file("/tmp/" + file_name, bucket_name, s3_key)
    waiter = s3.get_waiter('object_exists')
    waiter.wait(Bucket=bucket_name, Key=s3_key)
    #df_to_upload.to_csv("s3://proactive-data-analytics/" + s3_key )
...
def upload_df_to_s3_as_parquet(df_to_upload, file_name, file_type):
    ...
    """
    bucket_name = os.environ['BUCKET_NAME']
    s3_key = f'device_readings_auditing/{file_type}/{today}/{hour}/{file_name}'
